...
get_parent_iousb_device( io_object_t& serial_port )
{
    ...
    while (true)
    {
        ...
        if (name == "IOUSBDevice" || name == "IOUSBHostDevice")
        {
            break;
        }
...
vector<UFC::SerialPortInfo> SerialConfigManager::listPorts()
{
    ...
    while ( (serial_port = IOIteratorNext(serial_port_iterator)) )
    {
        ...
        port_info.type = SerialPortType::USB;
        port_info.serialNumber = rtrim(get_string_property( parent, "USB Serial Number" ) );
        port_info.vendorId = get_int_property( parent, "idVendor" );
        port_info.productId = get_int_property( parent, "idProduct" );
